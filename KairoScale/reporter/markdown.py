...
from KairoScale.reporter.charts import (
...
logger = get_logger("KairoScale.reporter.markdown")
...
def generate_report(
    ...
    if results:
        ...
        sections.append(_render_combo_results(configs, results))
    else:
...
def _render_header(config: RunConfig, mode: Optional[str]) -> str:
    ...
    return (
        f"# KairoScale Optimization Report\n\n"
...
def _render_proposed_optimizations(
    ...
    for c in configs:
        ...
        if config_export_dir is not None:
            ...
            lines.append(
                f"**Apply command**: `KairoScale apply {cfg_path} --repo {Path(run_config.repo_path).resolve()}`"
            )
...
def _combo_member_ids(config: OptimizationConfig) -> list[str]:
    for note in config.heuristic_rationale:
        if not note.startswith("Combo members:"):
            continue
        raw = note.split(":", 1)[1].strip()
        return [part.strip() for part in raw.split(",") if part.strip()]
    return []


def _render_combo_results(
    configs: list[OptimizationConfig],
    results: list[ValidationResult],
) -> str:
    combo_configs = [cfg for cfg in configs if cfg.id.startswith("combo-")]
    lines = ["## Combo Results\n"]
    if not combo_configs:
        lines.append("*No combo configs were generated.*")
        return "\n".join(lines)

    config_by_id = {cfg.id: cfg for cfg in configs}
    result_by_id = {result.config_id: result for result in results}

    lines.append(
        "| Combo ID | Combined Configs | Est. Compound Speedup | Measured Speedup | Status | Notes |"
    )
    lines.append(
        "|----------|------------------|------------------------|------------------|--------|-------|"
    )
    ...
    for combo in combo_configs:
        members = _combo_member_ids(combo)
        if members:
            combined = "; ".join(
                f"{mid} ({config_by_id[mid].name})" if mid in config_by_id else mid
                for mid in members
            )
        else:
            combined = "Unknown"

        result = result_by_id.get(combo.id)
        measured = "-"
        status = "NOT VALIDATED"
        notes = "No validation result available."
        if result is not None:
            if not result.success:
                status = "FAIL"
                notes = result.error or "Runtime failure"
            elif result.diverged:
                measured = f"{result.speedup_vs_control:.2f}x"
                status = "FAIL (diverged)"
                notes = result.divergence_reason or "Diverged during validation."
            else:
                measured = f"{result.speedup_vs_control:.2f}x"
                status = "PASS"
        ...
        lines.append(
            f"| {combo.id} | {combined} | {combo.estimated_speedup:.2f}x | "
            f"{measured} | {status} | {notes} |"
        )
...
def _render_appendix(
    ...
    lines.append(
        "\n---\n*Generated by KairoScale v0.1.0*"
    )
