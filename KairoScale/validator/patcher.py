...
logger = get_logger("KairoScale.validator.patcher")
...
def apply_config(repo_path: Path, config: OptimizationConfig) -> Path:
    ...
    patch_dir = Path(tempfile.mkdtemp(prefix=f"KairoScale_patch_{config.id}_"))
    ...
    shutil.copytree(
        ...
        ignore=shutil.ignore_patterns(
            "__pycache__", ".git", "*.pyc", ".KairoScale_*"
        ),
    )
    ...
    if config.config_overrides:
        overrides_path = patched_repo / ".KairoScale_overrides.json"
    ...
    if config.dependencies:
        extra_reqs = patched_repo / ".KairoScale_extra_requirements.txt"
